...
class de_wide_resnet50_2(nn.Module):
    def __init__(self, pretrained: bool = False, progress: bool = True, **kwargs: Any):
        super().__init__()
        kwargs['width_per_group'] = 64 * 2
        self.Module = _resnet('wide_resnet50_2', Bottleneck, [
                              3, 4, 6, 3], pretrained, progress, **kwargs)
        self.out_conv=None
        if "out_conv" in kwargs.keys():
            self.out_conv=torch.nn.Sequential()
            out_pars= kwargs["out_conv"]
            self.out_conv.add_module("l1",torch.nn.Conv2d(in_channels=out_pars["l1"][0],out_channels=out_pars["l1"][1],
                kernel_size=out_pars["l1"][2],stride=out_pars["l1"][3],padding=out_pars["l1"][4]))
            self.out_conv.add_module("l2",torch.nn.Conv2d(in_channels=out_pars["l2"][0],out_channels=out_pars["l2"][1],
                kernel_size=out_pars["l2"][2],stride=out_pars["l2"][3],padding=out_pars["l2"][4]))
            self.out_conv.add_module("l3",torch.nn.Conv2d(in_channels=out_pars["l3"][0],out_channels=out_pars["l3"][1],
                kernel_size=out_pars["l3"][2],stride=out_pars["l3"][3],padding=out_pars["l3"][4]))
    def forward(self, x: Tensor) -> Tensor:
        if self.out_conv == None:
            return self.Module(x)
        else:
            y = self.Module(x)
            for i in range(3):
                y[i]= self.out_conv[i](y[i])
            return y
